...
def compare_models(model1, model2, verbose=True, rtol=1e-5, atol=1e-8):
    """
    Compare two PyTorch models to check if they have identical parameters and buffers.
    ...
    Returns:
        bool: True if models have identical parameters, False otherwise
    """
    models_identical = True
    
    # First check that the models have the same structure
    model1_state_dict = model1.state_dict()
    model2_state_dict = model2.state_dict()
    
    # Check if the state dicts have the same keys
    model1_keys = set(model1_state_dict.keys())
    model2_keys = set(model2_state_dict.keys())
    
    if model1_keys != model2_keys:
        if verbose:
            print("Models have different state dict keys")
            print(f"Keys only in model1: {model1_keys - model2_keys}")
            print(f"Keys only in model2: {model2_keys - model1_keys}")
        return False
    
    # Check each parameter tensor for equality
    different_params = []
    for key in model1_keys:
        ...
        if not torch.allclose(param1, param2, rtol=rtol, atol=atol):
            if verbose:
                ...
                abs_diff = (param1 - param2).abs()
                max_diff = abs_diff.max().item()
                mean_diff = abs_diff.mean().item()
                
                nonzero_mask = param1 != 0
                if nonzero_mask.any():
                    rel_diff = (abs_diff[nonzero_mask] / param1[nonzero_mask].abs()).mean().item()
                    print(f"Values differ for {key}: max abs diff={max_diff:.6e}, mean abs diff={mean_diff:.6e}, mean rel diff={rel_diff:.6e}")
                else:
                    print(f"Values differ for {key}: max abs diff={max_diff:.6e}, mean abs diff={mean_diff:.6e}")
                
                # Print a small sample of differing values for debugging
                flat1 = param1.flatten()
                flat2 = param2.flatten()
                diff_indices = torch.where((flat1 - flat2).abs() > atol)[0]
                if len(diff_indices) > 0:
                    sample_size = min(5, len(diff_indices))
                    sample_indices = diff_indices[:sample_size]
                    for idx in sample_indices:
                        print(f"  Index {idx}: {flat1[idx].item()} vs {flat2[idx].item()}, diff={abs(flat1[idx].item() - flat2[idx].item())}")
    ...
    if verbose:
        if models_identical:
            print("Models have identical parameters and buffers")
        else:
            print(f"Models differ in {len(different_params)} parameters/buffers")
...
def get_original_model(model, zero_out_dict, device= 'cpu'):
    model.to('cpu')
    ...
    for layer_name, pruned_filters in zero_out_dict.items():
        ...
        if layer_type == "conv":
            ...
            out_indices_to_retain = [i for i in range(new_conv.out_channels) if i not in out_indices]
            in_indices_to_retain = [i for i in range(new_conv.in_channels) if i not in in_indices]
            #set all the weights to 0
            new_conv.weight.data = torch.zeros_like(new_conv.weight.data)

            source_weights = layer.weight.data.clone()

            for i, out_idx in enumerate(out_indices_to_retain):
                for j, in_idx in enumerate(in_indices_to_retain):
                    new_conv.weight.data[out_idx, in_idx] = source_weights[i, j]
            
            if layer.bias is not None:
                #set bias to 0
                new_conv.bias.data = torch.zeros_like(new_conv.bias.data)
                new_conv.bias.data[out_indices_to_retain] = layer.bias.data.clone()
            layer = new_conv
            ...
            new_num_features = layer.num_features + len(indices)
            new_layer = nn.BatchNorm3d(new_num_features)
            indices_to_retain = [i for i in range(new_layer.num_features) if i not in indices]
            #set all the weights to 0
            new_layer.weight.data = torch.zeros_like(new_layer.weight.data)
            new_layer.weight.data[indices_to_retain] = layer.weight.data.clone()

            if layer.bias is not None:
                new_layer.bias.data = torch.zeros_like(new_layer.bias.data)
                new_layer.bias.data[indices_to_retain] = layer.bias.data.clone()
                
            #set the running mean and variances
            new_layer.running_mean = torch.zeros_like(new_layer.running_mean)
            new_layer.running_mean[indices_to_retain] = layer.running_mean.clone()
            new_layer.running_var = torch.ones_like(new_layer.running_var)
            new_layer.running_var[indices_to_retain] = layer.running_var.clone()
            ...
            indices_to_retain = [i for i in range(new_layer.in_channels) if i not in indices]
            #set all the weights to 0
            new_layer.weight.data = torch.zeros_like(new_layer.weight.data)
            new_layer.weight.data[indices_to_retain] = layer.weight.data.clone()
            if layer.bias is not None:
...
def helper_train(unet, train_loader, optimizer, criterion, num_epochs, device, lmd=0, mu_lg=1, mu=1, include_lg_pw=False, include_lpw=False, global_unet=None, global_segmenter=None, method="FedAvg", get_total_time=False):
    ...
    #measure time using torch.cudaEvents
    if get_total_time:
    ...
    for epoch in range(num_epochs):
        ...
        for _, data in enumerate(train_loader):
            ...
            if get_total_time:
                ...
                time += start.elapsed_time(end) # in milliseconds
    ...
    return losses, dices, time/1000 if get_total_time else None
...
def smart_aggregation(client_models, zero_dicts, weights, device = 'cpu'):
    """
    This function aggregates the client models using a smart aggregation method.
    It first creates a global model and a temporary model. The temporary model is used to calculate the weights for each client model.
    The global model is then updated using the weights from the client models.
    """
    ...
    #check if all of these are indeed ones
    # assert all([torch.all(temp_param.data == 1) for temp_param in temp_model.parameters()])

    for zero_dict, weight in zip(zero_dicts, weights):
        for layer_name in zero_dict.keys():
            ...
            layer_type = None
            if isinstance(tensor_ref, torch.nn.Conv3d):
                layer_type = "conv"
            elif isinstance(tensor_ref, torch.nn.BatchNorm3d):
                layer_type = "bn"
            elif isinstance(tensor_ref, torch.nn.ConvTranspose3d):
                layer_type= "convT"
            else:
                print(f"Warning: Layer type {type(tensor_ref)} not supported for {layer_name}")
                continue
            ...
            for (filter_index, type) in zero_dict[layer_name]:
                if type==OUT:
    ...
    #make sure that all the parameters in temp_param are between 0 and 1
    assert all([torch.all(temp_param.data >= 0) for temp_param in temp_model.parameters()])
    assert all([torch.all(temp_param.data <= 1) for temp_param in temp_model.parameters()])
    ...
    for temp_param, global_param in zip(temp_model.parameters(), global_model.parameters()):
        ...
        global_param.data = global_param.data / (temp_param.data + 1e-8)
...
def train(method="FedAvg", model=None, num_clients=4, datadict_train=None, datadict_val=None, datadict_test=None, num_rounds=200, num_clients_per_round=4, global_model=None, criterion=None, optimizer=None, scheduler=None, device=0, size=(128, 128, 128), no_crop=False, no_aug=False ,num_epochs=200, hyperparams=None, lmd = 0, mu=0,  checkpoint=None, batch_size=8, not_all_clients=False, num_workers=4, use_simulated=False, custom=False, dropout_contrastive = False, temperature=0.07, additional_save_path=None, patience=20, additional_losses=[''], starting_lr = 1e-3, lr_decay_rate = 0.99, min_rounds=0, include_lg_pw=False, include_local_pw = False, include_every=True, lrs = None, option=1, mu_lg=1, weightage=1, base_prob=0.05, before_prune_epochs=5, after_prune_epochs=5, pruning_percentage=1, pruning_mode='Taylor', smart_aggregate=False, start_round_pruning=0, introduce_importance=False, importance_factor=[0.1], T=20, glm_dir = None):
    ...
    for client in datadict_train.keys():
        ...
        train_dataloaders_prune[client] = DataLoader(datadict_train[client], batch_size=1, shuffle=True, num_workers=num_workers, collate_fn=collate_fn)
    ...
    for round in range(start_round, num_rounds):
        ...
        for i in range(num_clients_per_round):
            ...
            client_unet = get_reduced_model(client_unet, client_zero_out_dict, device=device)
            client_unets[i] = client_unet
            client_optimizer = optim.Adam(list(client_unet.parameters()), lr = lrs[i], eps = 0.0001)


            train_losses, train_dices, t_time = helper_train(client_unet, train_dataloader, client_optimizer, criterion, before_prune_epochs, device, lmd, mu_lg, mu, include_lg_pw, include_local_pw, global_unet, method=method, get_total_time=True)
            ...
            client_unet = get_original_model(client_unet, client_zero_out_dict, device=device)
            client_unets[i] = client_unet
            t1 = time.time()
            ...
            # print("Zero out dict unet: ", zero_out_dict)
            ...
            t1 = time.time()
            train_losses, train_dices, t_time = helper_train(client_unet, train_dataloader, client_optimizer, criterion, after_prune_epochs, device, lmd, mu_lg, mu, include_lg_pw, include_local_pw, global_unet, method=method, get_total_time=True)
            t2 = time.time()
            train_time += t_time
            ...
            #test the local model on its test data
            params_remaining_cur_round += count_params(client_unet)
            model_size += get_model_size_mb(client_unet)

            test_loss, test_dice, inference_time = helper_validate(client_unet, test_dataloaders[clients[i]], criterion, device, return_inference_time=True)
            ...
            client_unets[i] = get_original_model(client_unet, client_zero_out_dict, device=device)
        ...
        print(f"Average train time: {avg_train_time:.4f} seconds.")
        #perform aggregation
