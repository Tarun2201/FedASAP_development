...
from models.unet_model_targeted_dropout import UNet1, Segmenter
...
if __name__ == "__main__":
    ...
    for i, char in enumerate(binary_before_after_string):
        ...
        unet = UNet1(init_features=16, drop_probs={i:0.005 for i in range(17)})
        ...
        predictions, ground_truth, test_dice, test_tpr, test_lesion_tpr, test_precision, test_f1_score, test_hd, test_hd95, test_inference_time = helper_test(unet, segmenter, test_dataloaders1[args.data[i]], device, option=option)
    
        results.append([args.data[i], test_dice, test_tpr, test_lesion_tpr, test_precision, test_f1_score, test_hd95, test_inference_time])
    ...
    for col in range(1, 8):  # extend range to include new metrics
    ...
    # print("Client\tRound\tDice\tTPR\tLesion-level TPR\tLesion-level Precision\tLesion-level F1 Score\tHD\t%Removed\t%UnetParams\t%SegParams\t%TotalParams\tAvgInferenceTime")
    ...
    print("Client\tRound\tDice\tTPR\tLesion-level TPR\tLesion-level Precision\tLesion-level F1 Score\tHD\tHD95\tAvgInferenceTime")
    for i in range(len(results)):
        for j in range(len(results[i])):
            num = results[i][j]
            if not isinstance(num, str) and not isinstance(num, int):
