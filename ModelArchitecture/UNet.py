...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        bottleneck = self.bottleneck(self.pool4(enc4)) #for MOON
        ...
        seg = self.seg(dec1)
        bottleneck = dec1 #for pixelwise contrastive losses

        # stacked_out = torch.concat([enc1,F.interpolate(enc2,size = self.image_shape), F.interpolate(enc3,size = self.image_shape),F.interpolate(enc4,size = self.image_shape)],dim=1)
        # print(stacked_out.shape)
        return self.outsig(seg),bottleneck
...
class NestedUNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, init_features = 16, deep_supervision=False, **kwargs):
        ...
        if self.deep_supervision:
            ...
            self.final4 = nn.Conv3d(nb_filter[0], out_channels, kernel_size=1)
        else:
    ...
    def forward(self, input):
        ...
        if self.deep_supervision:
            ...
            return [output1, output2, output3, output4], bottleneck
